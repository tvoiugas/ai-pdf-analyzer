...
PostgreSQL (pgvector) — база данных для хранения векторов.
Redis — брокер сообщений для Celery.
App — FastAPI сервер (доступен по адресу http://localhost:8000).
Worker — фоновый процесс для обработки тяжелых PDF-задач.

## 3. Настройка Ollama
Если вы используете Ollama локально, убедитесь, что она доступна для Docker-контейнеров, и модель загружена:
```bash
